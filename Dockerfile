FROM scratch
ADD ML8-shimbuild.tar.gz /
CMD ["/bin/bash"]
COPY shim-unsigned-x64.spec /builddir/build/SPECS/
COPY shim*.efi /
COPY rpmmacros /root/.rpmmacros
COPY mlsecureboot003.der /builddir/build/SOURCES/
COPY shim-find-debuginfo.sh /builddir/build/SOURCES/shim-find-debuginfo.sh
COPY dbx.esl *.patch sbat.ml.csv /builddir/build/SOURCES/
COPY shim-15.4.tar.bz2 /builddir/build/SOURCES/
